const repository = require('../repositories/estudante.repository');
const { sendList } = require('./list-response');

exports.listar = async (req, res) => sendList(res, await repository.list(req.query));

exports.buscarPorId = async (req, res) => {
  const estudante = await repository.findById(req.params.id);
  if (!estudante) return res.status(404).json({ erro: { codigo: 'ESTUDANTE_NAO_ENCONTRADO', mensagem: 'Estudante não encontrado.' } });
  res.json(estudante);
};

exports.criar = async (req, res) => res.status(201).json(await repository.create({ nome: req.body.nome, email: req.body.email, status: req.body.status || 'ativo' }));
exports.substituir = async (req, res) => res.json(await repository.update(req.params.id, { nome: req.body.nome, email: req.body.email, status: req.body.status || 'ativo' }));
